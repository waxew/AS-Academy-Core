    ...
    MaterialTheme(colorScheme = if (darkMode) darkColorScheme() else lightColorScheme()) {
        ...
        LaunchedEffect(courseId) {
            runCatching {
                ...
                loadedCourse to LearningExtrasLoader(context.assets).load(courseId)
    ...
    ModalNavigationDrawer(drawerState = drawer, drawerContent = {
        ...
        Scaffold(topBar = { TopAppBar(title = { Text(course.manifest.titleEn) }, navigationIcon = { Button(onClick = { scope.launch { drawer.open() } }) { Text("☰") } }) }) { padding ->
            NavHost(navController = nav, startDestination = AcademyRoutes.HOME, modifier = Modifier.padding(padding)) {
                ...
                composable(AcademyRoutes.LESSON) { back -> LessonScreen(course, back.arguments?.getString("lessonId").orEmpty(), db, codeRunner, extras, nav) }
                ...
                composable(AcademyRoutes.QUIZ) { back -> QuizDetailScreen(extras, back.arguments?.getString("quizId").orEmpty()) }
...
private fun HomeScreen(course: CoursePackage, extras: LearningExtras, nav: NavHostController) {
    LazyColumn(Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        item { AcademyCard("مرکز تمرین", "${extras.exercises.size} تمرین • ${extras.quizzes.size} آزمون • ${extras.projects.size} پروژه") { nav.navigate(AcademyRoutes.EXERCISES) } }
        items(course.levels.sortedBy { it.order }, key = { it.id }) { level ->
            val chapters = course.chaptersFor(level.id); val count = chapters.sumOf { course.lessonsFor(it.id).size }
            AcademyCard(level.title, "$count درس", onClick = { nav.navigate(AcademyRoutes.chapters(level.id)) })
        }
    }
}

@Composable private fun ChapterScreen(course: CoursePackage, levelId: String, nav: NavHostController) { LazyColumn(Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) { items(course.chaptersFor(levelId), key = { it.id }) { chapter -> AcademyCard(chapter.title, chapter.description, onClick = { nav.navigate(AcademyRoutes.lessons(chapter.id)) }) } } }
@Composable private fun LessonListScreen(course: CoursePackage, chapterId: String, nav: NavHostController) { LazyColumn(Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) { items(course.lessonsFor(chapterId), key = { it.id }) { lesson -> AcademyCard(lesson.title, "${lesson.estimatedMinutes} دقیقه", onClick = { nav.navigate(AcademyRoutes.lesson(lesson.id)) }) } } }

@Composable
private fun LessonScreen(course: CoursePackage, lessonId: String, db: AcademyDatabase, runner: CodeRunner?, extras: LearningExtras, nav: NavHostController) {
    val lesson = course.lesson(lessonId) ?: return MessageScreen("درس پیدا نشد", lessonId)
    val scope = rememberCoroutineScope(); var runOutput by remember { mutableStateOf<String?>(null) }
    val lessonExercises = extras.exercises.filter { it.lessonId == lessonId }; val lessonQuizzes = extras.quizzes.filter { it.lessonId == lessonId }
    LazyColumn(Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        items(lessonExercises, key = { "exercise:${it.id}" }) { exercise -> AcademyCard("تمرین: ${exercise.title}", exercise.description) { nav.navigate(AcademyRoutes.exercise(exercise.id)) } }
        items(lessonQuizzes, key = { "quiz:${it.id}" }) { quiz -> AcademyCard("آزمون: ${quiz.title}", "${quiz.questions.size} سؤال") { nav.navigate(AcademyRoutes.quiz(quiz.id)) } }
