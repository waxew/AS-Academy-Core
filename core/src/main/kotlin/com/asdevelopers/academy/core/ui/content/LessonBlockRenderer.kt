package com.asdevelopers.academy.core.ui.content
...
/**
 * Renderer پویا تمام LessonBlockهای قرارداد را نمایش می‌دهد؛ Courseها Screen درس جداگانه نمی‌سازند.
 */
@Composable
fun LessonRenderer(
    lesson: Lesson,
    ...
    LazyColumn(
        ...
        items(lesson.blocks, key = LessonBlock::id) { block ->
            RenderLessonBlock(block, onExerciseClick, onQuizClick, onProjectClick, assetRenderer)
        }
    }
}

/** هر نوع Block در یک when مرکزی نگهداری می‌شود تا رفتار همه اپ‌ها یکسان باشد. */
@Composable
fun RenderLessonBlock(
    block: LessonBlock,
    onExerciseClick: (String) -> Unit = {},
    onQuizClick: (String) -> Unit = {},
    onProjectClick: (String) -> Unit = {},
...
private fun SimpleTable(content: String) {
    // قالب سبک Pipe-separated برای نمایش Offline کافی است و Renderer تخصصی می‌تواند بعداً Slot شود.
        ...
        Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            content.lines().filter(String::isNotBlank).forEachIndexed { index, row ->
                ...
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                    cells.forEach { cell ->
                        Text(cell, modifier = Modifier.weight(1f), fontWeight = if (index == 0) FontWeight.Bold else FontWeight.Normal)
                    }
                }
            }
        }
...
private fun CodeBlock(content: String, language: String?) {
    Surface(
        ...
        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            SelectionContainer {
                Text(content, fontFamily = FontFamily.Monospace, style = MaterialTheme.typography.bodyMedium)
            }
        }
    }
}
...
@Composable
private fun Callout(content: String, icon: ImageVector, background: androidx.compose.ui.graphics.Color) {
    Row(
        modifier = Modifier.fillMaxWidth().background(background, RoundedCornerShape(12.dp)).padding(14.dp),
